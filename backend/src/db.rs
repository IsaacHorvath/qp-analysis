...
use crate::schema::speaker::dsl::speaker;
...
// pub fn get_gender_word_count(connection: &mut MysqlConnection, word: &str) -> Vec<BreakdownResponse> {
//     use crate::schema::gender::dsl::{gender, name, total_words};
//     use crate::schema::count_words;
//     
//     speech_clean
//         .filter(s_id.ne(117))
//         .inner_join(speaker.inner_join(gender))
//         .group_by((name, colour, total_words))
//         .select((
//             name,
//             colour,
//             sum(count_words(text, word)),
//             total_words
//         ))
//         .load::<(String, Option<i64>, i32)>(connection)
//         .expect("error loading party word count")
//         .into_iter()
//         .map(|row| { GenderBreakdownResponse {
//             name: row.0,
//             
//             count: row.1.unwrap() as i32,
//             score: 100000.0/(row.2 as f32)*(row.1.unwrap() as f32), // should this be in sql?
//         } })
//         .collect()
// }
